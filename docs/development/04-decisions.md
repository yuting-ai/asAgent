...
### DEC-039：文件系统能力采用最小路径范围与可解释扩权

- 日期：2026-08-09
- 状态：已确认，阶段 5 实现
- 背景：本地 Agent 若以用户主目录、整块磁盘或不透明的递归搜索作为默认能力，会使单一任务获得远超所需的私人文件访问范围；模糊的“访问文件”提示也无法让用户作出知情决定。
- 决策：文件工具默认仅可访问由 `WorkspaceResolver` 提供的允许根目录，且每次操作都必须绑定规范化后的具体目标路径与操作类型。路径穿越和符号链接不得逃逸允许根。访问 Workspace 外内容必须先由用户选择精确文件或根目录，或将文件导入 Workspace；Agent 提议的新增根、范围扩大、写入、删除、执行命令和读取敏感位置均需独立、可见的用户批准。不得以“访问所有文件”“访问主目录”或未限定递归搜索作为默认授权。
- 用户可见信息：批准界面必须显示操作类型、精确规范化目标或根目录、所请求权限、是否递归、影响摘要和保留期限；不能只显示工具或 Agent 的宽泛最大能力。
- 审计：所有改变文件的操作以及授权/拒绝/范围扩大都记录最小必要元数据（时间、Run、工具、操作、目标范围和决定），不把文件正文、Secret 或无关路径写入审计。
- 原因：最小权限既减少误操作和隐私暴露，也让拒绝、重试与审计结果可解释；精确范围使用户能区分“处理我选中的文件”与“扫描整台电脑”。
- 影响：阶段 5 的 WorkspaceResolver、File Tool、Policy 和 Approval API 必须共同执行该边界；阶段 2 的 `granted_permissions` 只是一层能力开关，不能单独授权任意路径。未来若支持用户选择的外部根目录，必须把其来源、范围和生命周期作为显式数据建模。
- 替代方案：默认允许整个用户目录、仅依赖模型提示词自律、在工具内部静默扩大路径范围、或以一次全局“磁盘访问”批准覆盖所有未来任务；当前均不采用。
