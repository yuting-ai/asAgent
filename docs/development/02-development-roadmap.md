...
- 文件操作默认仅限 Workspace 允许根；外部文件或根目录必须由用户精确选择或导入，不把主目录、整块磁盘或未限定递归搜索作为默认能力。
- 实现只读工具 `filesystem.read_file`、`filesystem.list`。
- 再实现受控写入工具。
- 建立 Tool Policy 和 Approval 接口；批准请求展示操作、规范化路径/根、权限、递归范围、影响摘要和有效期限，新增根或范围扩大单独批准。
...
- 不会因单一文件任务请求或获得主目录、整块磁盘或无关目录的默认访问；外部范围只能来自用户的精确选择或导入。
- 取消长任务后，下一次对话正常。
