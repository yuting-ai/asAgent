...
后续才在此 v1 前缀下单独定义分页以及 fetch-based SSE 的事件帧、`sequence` 续传和资源释放语义。当前后台执行只使用离线 `development-tools` Runtime；真实 Provider 的服务端配置、崩溃恢复/执行 claim 仍未实现。Electron 尚未接入前允许通过文档与契约测试审慎调整；Renderer 开始依赖后，任何破坏性修改必须新开版本或给出明确迁移策略。
